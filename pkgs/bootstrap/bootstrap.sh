...
function showSwitch() {
	local host
	if [[ $# != 1 ]]; then
		echo "${SCRIPT} ${CMD} <host>"
		return 1
	fi
	host=$1
	cat <<-EOF
		SSH_NIXOPTS="-t -i ~/.ssh/id_nixos-build" nixos-rebuild switch -j auto --use-remote-sudo --target-host ${host} --flake .#${host}
	EOF
}
...
function showInstall() {
	...
	if [[ $# != 4 ]]; then
		echo "${SCRIPT} ${CMD} <host> <poolName> <zfsDevice> <bootDevice>"
		return 1
	fi
	host=$1
	pool=$2
...
scp ~/.ssh/id_nixos-build root@${host}:~/.ssh/id_ed25519
nix run .#bootstrap zfs ${pool} ${zfsDevice} | ssh root@${host} -i ~/.ssh/id_nixos-build "cat > ~/stage1.sh"
nix run .#bootstrap mount ${pool} ${bootDevice} | ssh root@${host} -i ~/.ssh/id_nixos-build "cat >> ~/stage1.sh"
ssh root@${host} -i ~/.ssh/id_nixos-build "mkdir -p /mnt/nixos/persist/etc/ssh"
sops --decrypt --extract '["host_keys"]["${pool}"]' hosts/common/secrets.yaml | ssh root@${host} -i ~/.ssh/id_nixos-build "cat > /mnt/nixos/persist/etc/ssh/ssh_host_ed25519_key"
ssh root@${host} -i ~/.ssh/id_nixos-build "chmod 0600 /mnt/nixos/persist/etc/ssh/*"
echo "git clone https://github.com/NeilDarach/nix-config.git" | ssh root@${host} -i ~/.ssh/id_nixos-build "cat > ~/stage2.sh"
echo "cd nix-config | ssh root@${host} -i ~/.ssh/id_nixos-build "cat >> ~/stage2.sh"
echo "nixos-install --root /mnt/nixos --flake .#${pool}"| ssh root@${host} -i ~/.ssh/id_nixos-build "cat >> ~/stage2.sh"
echo "nix run .#bootstrap close ${pool} ${bootDevice} | ssh root@${host} -i ~/.ssh/id_nixos-build "cat >> ~/stage2.sh"
EOF
}
...
function showZfs() {
	local pool device
	if [[ $# != 2 ]]; then
		echo "${SCRIPT} ${CMD} <poolName> <zfsDevice>"
		return 1
	fi
	pool=$1
	device=$2
...
function showMount() {
	...
	if [[ $# != 2 ]]; then
		echo "${SCRIPT} ${CMD} <poolName> <bootDevice>"
		return 1
	fi
	pool=$1
	bootdevice=$2
