{ config, lib, inputs, ... }: {
  flake.modules = {
    nixos.svc-espresense = nixosArgs@{ pkgs, config, lib, ... }:
      let
        # Get the tags with
        ...
        tags = {
          aarch64-linux = {
            ...
            arch = "arm64";
            imageDigest =
              "sha256:444ffa6ef0f6dfce6f17a4b3b5bb148624eb53d05b93078c6e6916b378822c29";
          };
          ...
          "x86_64-linux" = {
            ...
            arch = "amd64";
            imageDigest =
              "sha256:2dad854a3dc4788ee8f213191b4e020de24f5fc375a1803ebbd8ade18995fad1";
          };
        }."${pkgs.stdenv.hostPlatform.system}";
        base = pkgs.dockerTools.pullImage (tags // {
      ...
      in {
        ...
        config = lib.mkIf config.local.espresense.enable {
          ...
          systemd.services.espresense = {
            ...
            serviceConfig = {
              ExecStartPre =
                "${pkgs.coreutils}/bin/chmod go-rw /strongStateDir/espresense/config.yaml";
            };
          };
        };
      };
