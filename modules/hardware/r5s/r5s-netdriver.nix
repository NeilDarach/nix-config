{ config, lib, pkgs, ... }: {
  flake.modules = {
    nixos.hardware-r5s-netdriver = nixosArgs@{ config, ... }: {
      options = {
        nanopi-r5s.network.r8125 =
          lib.mkEnableOption "custom r8124 network driver";
      };
      config = lib.mkIf config.nanopi-r5s.network.r8125 (let
