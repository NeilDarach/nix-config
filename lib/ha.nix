# Taken from https://github.com/nathan-gs/nix-conf/blob/main/lib/ha.nix
{ lib, ... }:
...
let
  ...
  genId = id:
    (lib.strings.toLower
      (builtins.replaceStrings [ "/" "." " " ] [ "_" "_" "_" ] id));
  ...
  sensor = {
    ...
    energy_demand_remaining = appliance: condition: template: {
      ...
      unique_id =
        genId "electricity/demand_management/${appliance}.energy_remaining";
      device_class = "energy";
      state = ''
        {% if ${condition} %}
          ${template}
    ...
    avg_from_list = name: sensors:
      { unit_of_measurement, device_class, adjustment ? 0, icon ? "" }: {
        name = name;
        state = ''
          {% set v = [
  ...
  automation = name:
    { triggers ? [ ], conditions ? [ ], actions ? [ ], mode ? "single" }: {
      id = genId name;
      alias = name;
      trigger = triggers;
  ...
  automationOnOff = name:
    { triggersOn ? [ ], triggersOff ? [ ], conditionsOn ? [ ]
    , conditionsOff ? [ ], entities ? [ ], mode ? "single" }: [
  ...
  automationToggle = name:
    { triggers ? [ ], conditions ? [ ], entities ? [ ], mode ? "single" }:
    [
  ...
  lightBrightness = name: value:
    { triggers ? [ ], conditions ? [ ], entities ? [ ], mode ? "single"
    , transition ? 0 }:
    [
      (automation "${name}.brghtness" {
        triggers = triggers;
        conditions = conditions;
        actions = map (entity: (action.set_brightness entity value transition))
          entities;
        mode = mode;
      })
    ];
  action = {
    ...
    set_brightness = entity: value: transition: {
      service = "${lib.head (lib.strings.splitString "." entity)}.turn_on";
      target.entity_id = entity;
